...
void calculate_distance_and_matches(vector<SiftDescriptor> &matches,
		vector<int> &distances, const vector<SiftDescriptor> descriptors,
			...
			if (question == "q3") {
				...
				for (int i = 0; i < filecount; i++) {
					cout << "image: " << images[i] << " \t|| matches: "
							<< max_matches[i] << "\n";
					if (i < 10 && (place.length() < images[i].length())
				...
				vector<vector<int> > fv = quantize_vectors(descriptors, x_vec,
						500.0, k_val);
				...
				for (int i = 0; i < filecount; i++) {
					...
					vector<vector<int> > f1v = quantize_vectors(descriptors2,
							x_vec, 500.0, k_val);
					...
					for (int j = 0; j < fv.size(); j++) {
						...
						for (int y = 0; y < f1v.size(); y++) {
							if (fv[j] == f1v[y]) {
								int temp = euclidean(descriptors[j],
										descriptors2[y]);
								if (temp < dist) {
									second_dist = dist;
									dist = temp;
					...
					cout << "Matches: " << match_ratio << " for image: "
							<< infile2 << "\n";
				}
			} else {
...
//			Algo:
//			take 2 images
//			sift gives descriptor pairs
//			randomly take 4 points
//				make 8x8 matrix (slide)
//				solver gives h
//				count inliers
